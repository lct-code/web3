...
export type ExtractedRBTMetadata = Partial<CreateRBTPayload> & {
  release_date?: string;
  album_name?: string;
};
...
function extractMeta(payload: Payload): Promise<ExtractedRBTMetadata> {
  return apiClient
    .post<Response>(`RBT/${payload.fileEntryId}/extract-metadata`, payload)
    .then(r => metadataToFormValues(r.data));
}

function metadataToFormValues(response: Response): ExtractedRBTMetadata {
  const values: ExtractedRBTMetadata = {
...
type Values = Partial<CreateRBTPayload>;
export function mergeRBTFormValues<
