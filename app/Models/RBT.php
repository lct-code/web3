...
class RBT extends BaseModel
{
    use OrdersByPopularity, HasFactory, Searchable;

    const MODEL_TYPE = 'RBT';
    ...
    {
        return $this->hasMany(RBTPlay::class);
    }
        ...
        return Attribute::make(
            get: function ($value) {
                ...
                if (
                    $value &&
                    $endpoint &&
                    str_contains($value, 'storage/RBT_media') &&
                    str_contains($value, 'https://')
                ) {
                    return preg_replace(
                        '/https:\/\/.*?(\/storage\/RBT_media\/(.+?\.[a-z0-9]+))/',
                        "$endpoint$1",
                        $value,
                    );
                }
