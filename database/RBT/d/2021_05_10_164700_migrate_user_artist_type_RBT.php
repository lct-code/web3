...
class MigrateUserArtistTypeRBT extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        if ( ! Schema::hasColumn('artist_r_b_t', 'artist_type')) return;

        DB::table('artist_r_b_t')->where('artist_type', User::class)
            ->chunkById(100, function(Collection $rows) {
                ...
                $groupedRows->each(function(Collection $group, int $userId) use($users) {
                    ...
                    DB::table('artist_r_b_t')->where('artist_id', $user->id)->update(['artist_id' => $userArtist->id]);
                });
            });
    }
