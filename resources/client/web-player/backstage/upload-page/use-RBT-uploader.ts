...
import {
  ExtractedRBTMetadata,
  useExtractTackFileMetadata,
} from '@app/admin/RBT-datatable-page/requests/use-extract-RBT-file-metadata';
...
import {generateWaveformData} from '@app/web-player/RBT/waveform/generate-waveform-data';
...
export interface RBTUploadMeta {
  isGeneratingWave?: boolean;
  isExtractingMetadata?: boolean;
}

export type RBTUploadPayload = CreateRBTPayload & {uploadId: string};

interface Options {
  autoMatchAlbum?: boolean;
  onUploadStart?: (data: RBTUploadPayload) => void;
  onMetadataChange: (
    file: UploadedFile,
    newData: ExtractedRBTMetadata & {waveData?: number[][]},
  ) => void;
}
export function useRBTUploader(options: Options) {
  ...
  const updateUpload = useCallback(
    (uploadId: string, newMeta: RBTUploadMeta) => {
        ...
        extractMetadata.mutate(
          ...
          },
          {
            onSuccess: formValues => {
              ...
              const newValues: ExtractedRBTMetadata = {
                ...formValues,
                src: entry.url,
              };
  ...
  const openFilePicker = useCallback(async () => {
    ...
    await uploadRBT(files);
  }, [uploadRBT, restrictions]);

  return {openFilePicker, uploadRBT, validateUploads};
}
