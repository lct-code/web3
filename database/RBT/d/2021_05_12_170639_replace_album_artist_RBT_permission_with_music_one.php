...
class ReplaceAlbumArtistRBTPermissionWithMusicOne extends Migration
{
    ...
    public function up()
    {
        ...
        $oldIds = $permissions->filter(function(Permission $permission) {
            return in_array($permission->group, ['artists', 'albums', 'r_b_t', 'lyrics', 'genres']);
        })->pluck('id');
        ...
        $toDelete = $permissions->filter(function(Permission $permission) {
            return in_array($permission->name, ['r_b_t.embed', 'r_b_t.play', 'r_b_t.download', 'mail_templates.view', 'mail_templates.update']);
        })->pluck('id');
