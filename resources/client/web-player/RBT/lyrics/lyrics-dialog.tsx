...
import {useLyrics} from '@app/web-player/RBT/lyrics/requests/use-lyrics';
...
export function LyricsDialog({RBT}: Props) {
  const {data, isLoading} = useLyrics(RBT);
      ...
      <DialogBody className="flex items-stretch gap-24 h-full" padding="p-0">
        <div className="hidden md:flex-[0.4_1_0%] md:flex items-center justify-end pl-40">
          <div>
            ...
            />
            <div className="mt-14 text-center text-xl">{RBT.name}</div>
            <div className="mt-4 text-center text-base text-muted">
              <ArtistLinks artists={RBT.artists} />
            </div>
          </div>
        </div>
