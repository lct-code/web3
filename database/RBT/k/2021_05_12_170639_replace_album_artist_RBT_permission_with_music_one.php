...
class ReplaceAlbumArtistRBTPermissionWithMusicOne extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        ...
        $oldIds = $permissions->filter(function(Permission $permission) {
            return in_array($permission->group, ['artists', 'albums', 'r_b_t_s', 'lyrics', 'genres']);
        })->pluck('id');
        ...
        $toDelete = $permissions->filter(function(Permission $permission) {
            return in_array($permission->name, ['r_b_t_s.embed', 'r_b_t_s.play', 'r_b_t_s.download', 'mail_templates.view', 'mail_templates.update']);
        })->pluck('id');
