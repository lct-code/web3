import {
  ...
  useLyrics,
  UseLyricsResponse,
} from '@app/web-player/RBT/lyrics/use-lyrics';
...
function Lyrics({RBT}: LyricsProps) {
  const duration = usePlayerStore(s => s.mediaDuration);
  const {data, isLoading} = useLyrics(RBT.id, {duration});
