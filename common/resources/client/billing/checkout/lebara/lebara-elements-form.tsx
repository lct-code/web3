...
import { useLebara, LebaraPayload } from './use-lebara';
...
interface LebaraElementsFormProps {
    productId?: string;
    priceId?: string;
...
export function LebaraElementsForm({
    priceId,
    type = 'subscription',
    submitLabel,
    verifyLabel,
    resendLabel,
    returnUrl,
}: LebaraElementsFormProps) {
    const { lebara, lebaraIsEnabled } = useLebara({
        type,
        priceId,
    });
    ...
    // disable upgrade button if lebara is enabled, but not loaded yet
    const lebaraIsReady: boolean =
        !lebaraIsEnabled || (lebara != null);
    ...
    const handleSubmit = async (input: LebaraPayload) => {
        // lebara has not loaded yet
        if (!lebara) return;
        ...
        try {
            ...
            const result = await lebara[method](payload);
            ...
            // don't show validation error as it will be shown already by lebara payment element
    ...
    const setSyncTimeout = (syncTimeoutIdx: number = 0) => {
        return setTimeout(() => {
            lebara.syncSubscriptionDetails().then((resp) => {
    ...
    return (
        <Form form={form} onSubmit={handleSubmit}>
            ...
            <Button
                ...
                type="submit"
                disabled={isSubmitting || !lebaraIsReady}
            >
